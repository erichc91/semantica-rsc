...
RESULTS_FILE = os.path.join(os.path.dirname(__file__), "..", "data", "v6_highpower_results.json")
...
def run_band(concepts: Dict, lang_embeddings_loaded: Optional[Dict],
    ...
    print(f"  Running permutation test (2000 permutations)...", flush=True)
    p_val = permutation_test(lang_embeddings, active, n_perm=2000)
...
def run():
    print("RSC Cross-Lingual Universality — v6 HIGH POWER (2000 permutations, 300 concepts/band)")
    print("=" * 65)
    print("Same concept count as v6 (300/band), 10x more permutations:")
    print("  - n_perm = 2000  (p floor = 1/2001 ~= 0.0005, vs 0.005 in v6)")
    print("  - band_size = 300 concepts (same as v6 — keeps O(n^2) tractable)")
    print("=" * 65)
    ...
    print(f"  Running permutation test (2000 permutations)...", flush=True)
    p_high = permutation_test(lang_embeddings_full, active_langs, n_perm=2000)
    ...
    for band_label, start, end in BANDS[1:]:
        ...
        print(f"  Running permutation test (2000 permutations)...", flush=True)
        p_b = permutation_test(band_embs, band_active, n_perm=2000)
